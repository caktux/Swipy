...
hammerPage.on('drag',function(e){if(self.Swipy.isEdgeDrag(e,options)||!Modernizr.touch){hammered.css({x:(e.gesture.deltaX+(e.gesture.direction=='left'?-options.edge_buffer:options.edge_buffer))*options.scale,scale:Math.max(options.scale,1-(Math.tan(Math.abs(e.gesture.deltaX)/$(window).width())))});}});hammerPage.on('dragstart dragend',function(e){if(options.debug)
